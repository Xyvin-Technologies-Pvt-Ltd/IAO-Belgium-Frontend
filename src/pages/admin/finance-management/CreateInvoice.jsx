      ...
      <div className="bg-white dark:bg-black border dark:border-white/20 rounded-xl shadow-lg w-full max-w-2xl max-h-[90vh] flex flex-col">
        <div className="p-6 border-b dark:border-white/20 flex items-start justify-between">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">
            {t("finance.reports.createInvoice.title", { defaultValue: "Create Invoice (Custom Payment)" })}
          </h2>
        ...
        <div className="overflow-y-auto flex-1 p-6">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div>
              <SearchableSelect
                label={t("finance.reports.createInvoice.student", { defaultValue: "Select Student" })}
                placeholder={t("finance.reports.createInvoice.searchStudent", { defaultValue: "Search for a student..." })}
            ...
            <div>
              <Label className="text-sm font-medium">
                {t("finance.reports.createInvoice.invoiceTitle", { defaultValue: "Invoice Title / Description" })}
              </Label>
              <Input
                type="text"
                placeholder={t("finance.reports.createInvoice.titlePlaceholder", { defaultValue: "e.g., Semester Fee, Scholarship Adjustment" })}
                {...register("title")}
                className="mt-1"
              />
            </div>
            ...
            <div>
              <Label className="text-sm font-medium">
                {t("common.currency", { defaultValue: "Currency" })} <span className="text-red-500">*</span>
              </Label>
              <select
            ...
            <div className="pt-4 flex justify-end gap-3">
              ...
              <Button type="submit" disabled={createPaymentMutation.isPending || !watch("user") || !watch("amount")}>
                {createPaymentMutation.isPending 
                  ...
                  : t("finance.reports.createInvoice.submit", { defaultValue: "Create Invoice & Mark Paid" })}
              </Button>
            </div>
          </form>
        </div>
