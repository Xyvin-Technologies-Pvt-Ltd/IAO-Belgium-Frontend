...
const FinanceReports = () => {
  ...
  // Calculate actual amount (excluding convenience fee for module-purchase)
          ...
          {isLoading ? (
            ...
            payments?.map((payment) => (
              <TableRow key={payment._id}>
                ...
                <TableCell>
                  {payment?.user
                    ? `${payment.user.first_name || ""} ${payment.user.last_name || ""}`.trim() || "N/A"
                    : "N/A"}
                </TableCell>
                ...
                <TableCell>
                  <span className={`font-medium ${getPurposeColor(payment?.purpose)}`}>
                    {formatPurpose(payment?.purpose)}
                  </span>
                </TableCell>
                <TableCell>
                  {payment?.currency || "EUR"} {getActualAmount(payment).toFixed(2)}
                </TableCell>
                <TableCell>
                  {payment.purpose === "module-purchase" && payment.convenience_fee
