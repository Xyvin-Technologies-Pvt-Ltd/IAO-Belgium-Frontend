...
const CreateCity = ({ open, onClose, cityData }) => {
  ...
  if (cityData?.country && !countries.find(c => c._id === cityData.country._id)) {
    countries.unshift(cityData.country);
  }
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() => appendVenue({ name: "", address: "" })}
                className="flex items-center justify-center w-8 h-8 text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md transition-colors"
              >
                <Plus className="h-4 w-4" />
              </button>
            ...
            {venueFields.map((field, index) => (
              <div key={field.id} className="flex flex-col gap-2 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                <div className="flex items-start gap-3">
                  <div className="flex-1 space-y-2">
                    <FormField
                      placeholder={`${t("cityManagement.modal.venueLabel")} ${index + 1}`}
                      error={errors.venue?.[index]?.name?.message}
                      {...register(`venue.${index}.name`)}
                    />
                    <FormField
                      placeholder={t("cityManagement.modal.venueAddress", "Venue Address (Optional)")}
                      error={errors.venue?.[index]?.address?.message}
                      {...register(`venue.${index}.address`)}
                    />
                  </div>
